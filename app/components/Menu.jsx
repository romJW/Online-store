...
function MenuItem({ children, className, onClick }) {
  return (
    <>
      <div
        onClick={onClick}
        className={`font-bold cursor-pointer text-white text-sm xl:text-base flex items-center lg:px-6 xl:px-8 hover:bg-[#8C1A1E]  ${className}`}>
        {children}
      </div>
    </>
  );
...
function Bar({}) {
  return <div className="mt-1 h-[30px] border-[0.2px] opacity-50 bg-[#FFFFFF66] "></div>;
}
    ...
    <>
      <div className="header w-full h-10 sticky mb-[-40px] top-[-1px] z-50  mx-auto  flex justify-center bg-[#21212199] items-center mix-blend-normal backdrop-blur-sm hidden lg:flex">
        <div className="hidden container font-bold md:flex justify-center mx-auto text-sm xl:text-base ">
          <div
            className={`cursor-pointer dropdown dropdown-hover  ${
              isCatalogHover ? 'bg-[#8C1A1E]' : ''
            }`}
            ...
            <div tabIndex={0} className="dropdown-content shadow-2xl z-20 w-[230px]">
              {nav.map((option) => {
                return (
                  <>
                    <div className={`flex bg-white text-black text-sm p-3 gap-2.5 dropItemShadow`}>
                      <img
                        src={option.logo}
                        dais
                        style={{
          ...
          <MenuItem className={`dropdown dropdown-hover `}>
            ...
            <div tabIndex={0} className="dropdown-content left-0 top-10 shadow w-[230px]">
              {companyItems.map((item) => (
                <Link to={item.path}>
                  <DropItem key={item.name} text={item.name} />
                </Link>
              ))}
            </div>
          </MenuItem>
          ...
          <div className="justify-center flex ">
            ...
            <MenuItem className="xl:px-8  2xl:px-10 " onClick={() => setBasketOpen(true)}>
              <img
                src="/assets/whiteBasket.svg"
                alt=""
                className=" "
                style={{ maxWidth: '30px' }}
              />
              <div className="w-3 h-3 bg-[#F12028] rounded-[10px] text-white text-[6px] relative bottom-2 z-10 flex justify-center items-center">
                {BasketArr.length > 100 ? '9+': BasketArr.length}
              </div>
            </MenuItem>
            <MenuItem className="whitespace-nowrap" onClick={() => setPartnership(true)}>
