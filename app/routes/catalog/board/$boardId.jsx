...
export default function BoardType() {
 ...
 const BoardHeader = ({title, size, className}) => {
  return (
    <h3 className={`font-[Days] uppercase text-3xl lg:text-5xl text-black ${className}`}>{title} {size} </h3>
  )
}
 ...
 const BoardSize = ({size}) => {
  ...
  return (
    ...
    <p className='text-black font-medium text-sm lg:text-lg mb-2'>Размер/Габариты:</p>
     <div className='rounded-[80px] flex justify-center items-center text-center bg-[#E4E4E4B2] text-[#6F6F6F] text-sm lg:text-lg w-20 lg:w-[100px] h-12 duration-300 hover:bg-[#6F6F6F99]'>{size}</div>
     </div>
...
const BoardCounter = ({title, state, minusEvent, plusEvent}) => {
  return(
    <div className='relative text-[#000000]'>
              ...
              <span className='text-lg absolute top-[35px] left-[5px] font-bold cursor-pointer' onClick={minusEvent}>-</span>
              <span className='text-lg absolute  top-[35px] left-[110px] font-bold cursor-pointer' onClick={plusEvent}>+</span>
            </div>
  )
}
    ...
    <div className='w-[320px] xl:w-[370px] bg-[#FDF7F2] rounded-[20px] py-8'>
      <div className='flex flex-col justify-center items-center'>
          <div className='flex gap-8'>
            <BoardCounter title='Погонные метры' minusEvent={()=>setSize(--size)} plusEvent={()=>setSize(++size)} state={size}/>
          <BoardCounter title='Квадратные метры' minusEvent={()=>setSquareMeter(--squareMeter)} plusEvent={()=>setSquareMeter(++squareMeter)} state={squareMeter}/>
          </div>
          ...
          <p className='text-black text-3xl font-bold uppercase mb-5' ref={el}>{size*395} ₸</p>
...
const AvailableColors = ({colors}) =>{
  return(
    ...
    {colors.map(color => <img src={`/assets/${color}`} className='w-12 h-12 rounded hover:scale-110'/>)}
      </div>
     </div>
  )
}
...
const BoardText = () =>{
  return(
    <div className='flex flex-col gap-4 text-base text-black w-[280px] xl:w-[370px]'>
      ...
      <p>Брашируется только одна сторона - на усмотрение заказчика. (По умолчанию оборотная сторона остается гладкая, по желанию может быть дополнительно браширована - уточняйте у менеджеров).</p>
  ...
  return(
    <div className='flex flex-col justify-center text-black w-[360px] mx-auto'>
      <div className="flex gap-2 items-center">
        ...
        <p className='text-sm lg:text-base mb-5'>Мы подготовили для Вас статью «5 ошибок монтажа террасной доски», которая поможет избежать частых ошибок и быть спокойным, что все нюансы учтены.</p>

        <Btn className='btn border-2 btn-neutral font-semibold text-base text-[#AC9B9C] border-[#AC9B9C] w-[330px] h-16'>Скачать в формате Pdf (2,3 Мб)</Btn>
    </div>
  )
}
  ...
  return(
    <div>
      <div className='flex flex-col text-black gap-8'> 
        {conditions.map(condition=><BoardItem icon={condition.icon} text={condition.text}/>)}
      </div>
    </div>
  )
}
    ...
    <div className='bg-[#FDF7F2]  w-[320px] xl:w-[370px] py-4 lg:py-8 px-8 rounded-[20px]'>
      <div className='flex flex-col justify-center items-center text-black gap-5 '> 
            <div className='flex gap-2 lg:gap-4'>
              ...
              <p><span className='font-bold text-base lg:text-lg'>Получите помощь <br/> специалиста</span> расчетом стоимости вашего проекта</p>
            </div>
            <Btn className='btn border-2 font-semibold bg-transparent border-[#E41D24] text-[#E41D24] hover:bg-[#E41D24] hover:text-white hover:border-none w-[270px] h-16'>Рассчитать проект</Btn>
      </div>
    </div>
  )
}
const BoardInstance = () => {
  return(
    ...
    <div className="flex flex-col justify-center items-center gap-5 text-base lg:text-lg text-center">
        ...
        <p className='font-bold px-8'>Хотите посмотреть <br/>образцы вживую?</p>
        <p className="px-8">Оставьте запрос на получение комплектов в своем городе</p>
        <Btn className='btn border-2 font-semibold bg-transparent border-[#E41D24] text-[#E41D24] hover:bg-[#E41D24] hover:text-white hover:border-none w-[270px] h-16'>Получить образец</Btn>
    </div>
    </div>
  )
}
...
const BoardSliderTop = () => {
  ...
  return(
<>
    <div className="mx-auto w-full relative text-black xl:w-[370px]">
    ...
    <img src="/assets/chevron.svg" className="btn-left  absolute bottom-[-50px] left-0 rotate-180"/>
    <img src="/assets/chevron.svg" className="btn-right absolute bottom-[-50px] right-0"/>
    {/* <i className="fa-solid fa-chevron-left text-[#21286433] fa-2x btn-left absolute bottom-[-50px] left-0"/>
      <i className="fa-solid fa-chevron-right text-[#21286433]  fa-2x  btn-right absolute bottom-[-50px] "/> */}
  </div>
  </>
  )
}
const BoardSliderBottom = () => {
  ...
  return(
<>
    <div className="mx-auto w-full relative text-black w-[280px]">
    <div className="swiper swiperBoardBottom mx-auto relative ">
      <div className="swiper-wrapper mx-auto">
        {slides.map((slide, i) => {
          return (
            <div key={slide.image} className="swiper-slide swiper-opacity flex gap-2 items-center justify-center overflow-hidden">
              <div className="w-[90px] h-[70px]"><img src={slide.image}  alt='' onClick={()=>setSliderState(i)}/></div>
            </div>
          );
        })}
      </div>  
  ...
  return(
    <>
    <CatalogLayout>
    <BreadCrumbs className="text-black" pages={[{name:'Главная', path:'/'}, {name:'каталог ДПК', path:'/catalog'},{name:'террасная доска', path:'/catalog/board'},{name:id, path:`/catalog/board/${id}`}]}/>
